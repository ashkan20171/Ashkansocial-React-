...
const DRAFT_KEY = 'ashkan.newpost.draft.v2';
...
export default function NewPost() {
  ...
  useEffect(() => {
    try {
      ...
      if (d?.image) {
        setImage(String(d.image));
        setShowImage(true);
      }
      if (d?.content || d?.image) toast.show(locale === 'fa' ? 'پیش‌نویس بازیابی شد' : 'Draft restored', 'info');
    } catch {}
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      try {
        const payload = { content, image: showImage ? image : '', ts: Date.now() };
  ...
  const suggestions = useMemo(() => {
    ...
    return list.filter((x) => x.toLowerCase().startsWith(q)).slice(0, 8);
  }, [token, users, tags]);
          ...
          <CardContent>
            <Stack gap={1.2}>
              <Typography sx={{ fontWeight: 950 }}>
                {locale === 'fa' ? 'ایجاد پست' : 'Create post'}
              </Typography>
              ...
              <TextField
                ...
                placeholder={locale === 'fa' ? 'متن پست…' : 'Write your post…'}
                fullWidth
                multiline
                minRows={5}
                inputRef={inputRef}
              ...
              {showImage && (
                ...
                >
                  <Box component="img" src={image} alt="preview" sx={{ width: '100%', display: 'block' }} />
                  <IconButton
                    onClick={() => {
                      setShowImage(false);
                      setImage('');
